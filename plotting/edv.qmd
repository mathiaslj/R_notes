...
- [ggtext](https://wilkelab.org/ggtext/) for ways to fx. color text labels, titles, etc. See an example in [the slides](https://www.stat.auckland.ac.nz/~paul/stats787/Current/Labs/design.html) of lecture 6.
...
*Channels* are a concept from [section 1.5](https://socviz.co/lookatdata.html#channels-for-representing-data) from Kieran Healey. See a list of visual channels in the figures to the right in the reference. **NOTE!** that the listing in the reference is ordered from top-to-bottom of most effective to least effective, where effectiveness is based on surveys of asking people to decode data values from their visual channel.
